...
function NavHead({ setToggleNav }: NavHeadProps) {
  return (
    <div className="w-full h-full absolute flex flex-col top-0 right-0 bg-white p-10">
      <button type="button" onClick={() => setToggleNav(false)}>
        <svg
          width="40"
          height="40"
          viewBox="0 0 20 20"
      ...
      <div className="mt-20 space-y-2">
        <ul className="space-y-4 font-bold">
          <NavLink url={'/shop'} label={'Shop'} />
          <NavLink url={'/onsale'} label={'On Sale'} />
          <NavLink url={'/newarrival'} label={'New Arrival'} />
          <NavLink url={'/brands'} label={'Brands'} />
        </ul>
      </div>
    </div>
  );
